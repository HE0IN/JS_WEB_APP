...
//응답객체의 주요 속성
//req.download   : 파일을 다운로드할 때 사용함.
//res.end        : 응답 프로세스를 종료함.
//res.json       : JSON 응답을 전송함. (Content-Type 헤더를 자동으로 만들어줌)
//res.jsonp      : JSONP 지원을 통해 JSON 응답을 한다.
//                 JSONP란 CORS(크로스 오리진)문제를 피하기 위해 별도의 방식으로 JSON을 가져오는 방식
...
//res.sendStatus : 상태 메세지와 함께 HTTP 상태 코드를 전송한다.
