...
const getAllContacts = asyncHandler(async (req, res) => {
    const contact = await Contact.find();  //sql select * from table; 이랑 같음
    res.status(200).send(contact); //JS KV 객체 => JSON 문자열로 변환되어 전송됨
});
...
const updateContact = asyncHandler( async (req, res) => {
    ...
    if (!contact) {
        res.status(404);
        throw new Error("연락처를 찾지 못함. (ID에 맞는 데이터가 없음)");
    }
    //수정 데이터 설정
    contact.name = name;
    contact.email = email;
